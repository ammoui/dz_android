...
class MediaDetailViewModel @Inject constructor(
    ...
    private fun load() {
        ...
        loadJob = viewModelScope.launch {
            ...
            try {
                val detail = repository.loadMediaDetailOrFavourite(type, id)
                if (detail != null) {
                    currentDetail = detail
                    uiState = MediaDetailUiState.Content(detail, isFavourite = false)
                    startObserveFavourite(detail)
                } else {
                    uiState = MediaDetailUiState.Error("Не удалось загрузить детали")
                }
            } catch (cancellationException: CancellationException) {
                throw cancellationException
            } catch (http: HttpException) {
                uiState = MediaDetailUiState.Error("Ошибка сервера (${http.code()})")
            } catch (io: IOException) {
                uiState = MediaDetailUiState.Error("Проблема с интернетом. Проверьте соединение.")
